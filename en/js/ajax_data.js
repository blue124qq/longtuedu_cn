function GetJson(op) {
    ...
    var jsonobj = JSON.parse("[" + $.ajax({ url: "/en/ajax/GetJson.ashx", data: op, async: false }).responseText + "]");
    return jsonobj;
};
    ...
    $.each(json, function (i, J) {
        if (J.Count > 0) {
            ...
            + '<div class="zlbj-listbox">'
             + '<a href="##"><div class="img" style="background-image: url(' + J.ImageUrl + ');"></div></a>'
             + '<div class="number">'
             ...
             + '<div class="text">'
              + '<h4>' + J.Title + '</h4>'
               + '<p>' + J.Summary + '</p>'
              + ' <a href="/contents/' + J.NodeID + '/' + J.ID + '.html"></a>'
             + '</div>'
            + '</div>'
            + '</div>';
...
function paging_list(count, op, ShowCount, page) {
    ...
    //    }
    if (page > 1) {
        li += "<li><a href='javascript:void(0)' onclick='" + op + "(" + (page - 1) + ")'>Pre</a></li> ";
    }
//    if (page > 2) {
//        li += "<li><a href='javascript:void(0)' onclick='" + op + "(" + (page - 1) + ")'>...</a></li> ";
//    }
    if (page > 1) {
...
//    }
    if (Math.floor(count / ShowCount) - page > 0) {
        li += "<li><a href='javascript:void(0)' onclick='" + op + "(" + (page + 1) + ")'>Next</a></li> ";
    }
    $(".paging-list").html(li);
}
function Toshoollist_go_page(page) {
    ...
    if ($("#s_province").val() != '' & $("#s_province").val() != 'Province') {
        if ($("#s_city").val() != '' & $("#s_city").val() != 'City') {
    ...
    Toshoollist(GetJson({ "command": "ContentList", "siteID": 73, "NodeName": "" + NodeName + "", "Page": page, "ShowCount": "9", "where": "" + str1 + str2 + str3 + str4 + "" }));
}
function Tostudentlist_go_page(page) {
    ...
    Tostudentlist(GetJson({ "command": "ContentList_bysfc", "siteID": 73, "NodeName": "" + NodeName + "", "Page": page, "ShowCount": "12", "where": "" + str1 + str2 + str3 + str4 + "" }));
}
...
function Tozplist(json) {
    ...
    $.each(json, function (i, J) {
        if (J.Count > 0) {
            ...
            li += '<div class="col-lg-12 pc-bsl-zp-listbox"><div class="row"><div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">'
            + '<p>' + J.Title + '</p>'
            ...
            + '</div><div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">'
            + '<p>' + J.AddDate + '</p>'
            + '</div><div class="col-lg-2 col-md-2 col-sm-2 col-xs-12"><p><a href="/en/contents/' + J.NodeID + '/' + J.ID + '.html" class="btn">Apply</a></p>'
            + '</div></div></div>';
...
function GetSchoolName(s,json) {
    var str = "<option value=\"\">Select School</option>";
    $.each(json, function (i, J) {
        str += "<option value=\"" + J.Name + "\">" + J.Name + "</option>";
    });
    $("#school_type").html(str);
    $("#school_type").val(s);
}
function GetPositionName(s,json) {
    var str = "<option value=\"\">Position</option>";
    $.each(json, function (i, J) {
        str += "<option value=\"" + J.Name + "\">" + J.Name + "</option>";
    });
    $("#Position").html(str);

$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "doc,docx,rar,zip,pdf,png,jpg", //文件类型
        filesize: "2048", //文件大小
        btntext: "浏览...", //上传按钮的文字
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            var btnObj = $('<div class="upload-btn">' + p.btntext + '</div>').prependTo(parentObj);
            //var btnObj = $(parentObj.find(".upload-btn"));
            ...
            if (p.FolderName != "") {
                p.sendurl += "&FolderName=" + p.FolderName;
            }
            if (!p.multiple) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        parentObj.children(".this-upload-path").val(data.path);
                        parentObj.children(".this-upload-name").text(data.name);
                        if (parentObj.children(".this-upload-path").attr("id") == "img_path") {
                            imagechange(data.path);
                        }
                    ...
                    var progressObj = parentObj.children(".upload-progress");
                    alert("上传成功");
                    progressObj.children(".txt").html("上传成功：" + file.name + file.path);
...
function setFocusImg(obj) {
    var focusPhotoObj = $(obj).parents(".photo-list5").siblings(".focus-photo");

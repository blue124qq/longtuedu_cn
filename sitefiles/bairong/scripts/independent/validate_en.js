...
function checkAttributeValueByParam(attributeName, isRequire, value, errorMessage, displayName, minNum, maxNum, validateType, regExp) {
    ...
    if (isRequire == "true" && (!value || value.length == 0)) {
        if (errorMessage.length > 0) {
            ...
            document.getElementById(attributeName + "_msg").innerHTML = displayName + "Can not be empty";
        }
    ...
    if (minNum > 0 && value.length < minNum) {
        if (errorMessage.length > 0) {
            ...
            document.getElementById(attributeName + "_msg").innerHTML = displayName + 'Length must not be less than' + minNum + 'character';
        }
        document.getElementById(attributeName + "_msg").style.display = "";
        return false;
    }
    ...
    if (maxNum > 0 && value.length > maxNum) {
        if (errorMessage.length > 0) {
            ...
            document.getElementById(attributeName + "_msg").innerHTML = displayName + 'Length can not be greater than' + maxNum + 'character';
        }
        document.getElementById(attributeName + "_msg").style.display = "";
        return false;
    }
            ...
            if (regExp.length > 0) {
                ...
                if (isChecked == false) {
                    if (errorMessage.length > 0) {
                        ...
                        document.getElementById(attributeName + "_msg").innerHTML = displayName + 'The format is incorrect';
                    }
                    document.getElementById(attributeName + "_msg").style.display = "";
                    return false;
                }
